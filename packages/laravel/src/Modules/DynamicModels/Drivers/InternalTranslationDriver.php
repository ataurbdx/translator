...
class InternalTranslationDriver implements TranslationDriverInterface
{
    ...
    public function get(mixed $target, string $field, ?string $locale = null, mixed $default = null): mixed
    {
        ...
        if ($model->relationLoaded('translatorEngineDynamics')) {
            $record = $model->translatorEngineDynamics->where('name', $field)->first();
        ...
        // 2. Query with cache
        $cacheKey = "translator_engine_internal_" . get_class($model) . "_{$model->getKey()}_{$field}";
        $ttl = config('translator_engine.cache.ttl', 86400);

        $translations = Cache::remember($cacheKey, $ttl, function () use ($model, $field) {
            $record = TranslatorEngineDynamic::where('translatable_type', $model->getMorphClass())
    ...
    public function set(mixed $target, string $field, string|array $localeOrValues, mixed $value = null): bool
    {
        ...
        $record = TranslatorEngineDynamic::firstOrNew([
    ...
    public function delete(mixed $target, ?string $field = null): bool
    {
        ...
        $query = TranslatorEngineDynamic::where('translatable_type', $model->getMorphClass())
            ->where('translatable_id', $model->getKey());

        if ($field) {
            $query->where('name', $field);
            Cache::forget("translator_engine_internal_" . get_class($model) . "_{$model->getKey()}_{$field}");
        } else {
            foreach ($model->translatable ?? [] as $f) {
                Cache::forget("translator_engine_internal_" . get_class($model) . "_{$model->getKey()}_{$f}");
            }
        }
