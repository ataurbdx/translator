# Master Translation Engine (`ataurbdx/translator-engine`)
## Architecture, Strategy & Implementation Blueprint

---

### Executive Vision
The **Master Translation Engine** is a high-performance, universal translation and localization ecosystem for Laravel and multi-platform applications (MERN, Next.js, Vue, mobile apps). 
...
| **Dynamic Models** | **2** | `TranslationInternal` | `internal` | **Shared Polymorphic Table**: Single shared table for multiple small-to-medium models (`Category`, `Department`, `FaqCategory`). Keeps database migrations clean. | Single common table (`translator_engine_dynamics`) via `morphTo` |
| **Dynamic Models** | **3** | `TranslationExternal` | `external` | **Dedicated Table Per Model**: Dedicated translation table for high-traffic, high-volume models (`Listing`, `Product`, `Post`). Maximum performance and composite indexing. | Individual table per model (`translator_engine_listings`) |
| **Dynamic Models** | **4** | `TranslationHybrid` | `hybrid` | **Grouped Domain Tables with Morph/Type**: A dedicated domain table shared across a related family of models (e.g. World cluster: `Country`, `Division`, `City`, `SubCity` sharing `translator_engine_worlds`). | Dedicated domain table partitioned by `entity_type` + `entity_id` |
| **Static UI Strings** | **5** | `TranslationStatic` | `static` | **Database-Driven UI Keys**: UI buttons, labels, menus, alerts. Stored in DB, managed via Admin UI, cached in Redis/Memory. Called via `{{ te('welcome') }}`. Zero file export required. | Database table (`translator_engine_statics`) + Redis/Memory Multi-Tier Cache |
...
| **Cultural Formatting** | **8** | `TranslationLocal` | `local` | **Cultural & Regional Formatter**: Localized digits (`১২৩`), calendar dates & months, South Asian grouping (`১২,৩৪,৫৬৭`), and invoice number-to-words (`এক হাজার পাঁচশত টাকা মাত্র`). | Database-driven table (`translator_engine_locales`) cached in Redis/Memory + fallback file |
| **Automation & AI** | **9** | `TranslationAI` | `ai` | **AI On-Demand / Background Sync**: Automatic translation of missing static or dynamic text using Google Gemini, OpenAI, Claude, or DeepL. | Queue jobs / AI Service Drivers writing into `translator_engine_statics` / `translator_engine_dynamics` |
| **Cross-Platform** | **10** | `TranslationAPI` | `api` | **Headless / REST / Cross-Platform**: Real-time REST endpoints for React, Next.js, Vue, mobile apps (Flutter, React Native) with ETag caching and batch fetching. | REST API Controller `/api/v1/translator-engine/...` |
...
* **For Static UI Strings (`static`)**:
  The default is stored directly in the `default` column of `translator_statics`. If a key has no translation for the active locale, it falls back to `default`.
...
* **Table**: `translator_engine_dynamics` (`translatable_type`, `translatable_id`, `field`, `value` JSON).
* **Auto-Created**: Installed automatically via `php artisan translator-engine:install`.
* **Model Configuration**:
  ```php
...
* **Table**: `translator_engine_{table}` (e.g., `translator_engine_listings`).
* **Generation Command**:
  ```bash
  php artisan translator-engine:make:external listings
  ```
  *Reads `listings` columns, lets you pick translatable fields, and generates `create_translator_engine_listings_table` migration automatically!*
* **Model Configuration**:
  ```php
...
* **Table**: `translator_engine_{domain}` (e.g., `translator_engine_worlds`, `translator_engine_catalog`).
* **Generation Command**:
  ```bash
...
* **Schema Design**:
  ...
  * `default`: Natural fallback string (e.g. `Add to Cart`).
...
* **Direct Database Printing (Via Key)**:
  ```blade
  {{ te('button.add_to_cart') }}
  {{-- Or full helper name: --}}
  {{ translator_engine('button.add_to_cart') }}
  ```
  - Resolves `bn` ➔ `"কার্টে যোগ করুন"`
  - If missing in `bn`, automatically falls back to `default` ➔ `"Add to Cart"`!
* **Export to JSON File Behavior**:
  - Uses `default` as the JSON file key: `"Add to Cart": "কার্টে যোগ করুন"`.
...
* **Usage**:
  ...
  ```
* **Bi-Directional Sync**:
  - `php artisan translator-engine:export-json` dumps DB static strings to JSON files.
  - `php artisan translator-engine:import-json` imports JSON files into the database.
...
Cultural formatting rules are stored in the database (`translator_engine_locales`) and cached in memory.

### Available Helper Functions:
```blade
{{-- 1. Localized Digits --}}
{{ te_digits('2026') }}           {{-- Returns: ২০২৬ (auto bn) --}}
{{ te_digits('2026', 'en') }}     {{-- Returns: 2026 --}}

{{-- 2. Localized Numbers & South Asian Grouping --}}
{{ te_number(1234567) }}          {{-- Returns: ১২,৩৪,৫৬৭ (auto bn) --}}
{{ te_number(1234567, 0, 'en') }} {{-- Returns: 1,234,567 (en) --}}

{{-- 3. Localized Dates & Months --}}
{{ te_date($order->created_at) }}        {{-- Returns: ০৯ ফেব্রুয়ারি ২০২৬ --}}
{{ te_date($order->created_at, true) }}  {{-- Returns with localized time --}}

{{-- 4. Financial Number-To-Words --}}
{{ te_words(1500) }}               {{-- Returns: এক হাজার পাঁচশত টাকা মাত্র --}}
{{ te_words(1500, 'USD', 'en') }}  {{-- Returns: One Thousand Five Hundred USD Only --}}
```
...
### Central Settings Table: `translator_engine_settings`
...
* **Drivers**: Google Gemini, OpenAI GPT-4o-mini, Claude, DeepL, Google Cloud Translate.
* **Capabilities**:
...
* **Headless REST Endpoints**:
  * `GET /api/v1/translator-engine/static?locale=bn`
...
├── composer.json
├── config/
│   └── translator_engine.php
├── src/
│   ├── TranslatorEngineServiceProvider.php
│   │
│   ├── Core/
│   │   ├── TranslatorEngine.php               # The Master Hub
│   │   ├── Contracts/                         # Engine Interfaces
│   │   ├── Facades/TranslatorEngine.php       # The Master Facade
│   │   ├── Traits/HasTranslatorEngine.php     # Unified Model Trait
│   │   └── Helpers/helpers.php                # te(), te_digits(), te_number(), te_words()
│   │
│   ├── Modules/
│   │   ├── DynamicModels/                     # Module 1 (Types 1 - 4)
│   │   │   ├── Drivers/                       (Inline, Internal, External, Hybrid)
│   │   │   ├── Models/                        (TranslatorEngineDynamic, TranslatorEngineHybrid)
│   │   │   └── Migrations/                    (create_translator_engine_dynamics_table)
│   │   │
│   │   ├── StaticUI/                          # Module 2 (Types 5 - 7)
│   │   │   ├── Drivers/                       (Static, Json, File)
│   │   │   ├── Models/                        (TranslatorEngineStatic)
│   │   │   └── Migrations/                    (create_translator_engine_statics_table)
│   │   │
│   │   ├── CulturalLocale/                    # Module 3 (Type 8)
│   │   │   ├── Drivers/                       (LocalDriver)
│   │   │   ├── Models/                        (TranslatorEngineLocale)
│   │   │   ├── Formatters/                    (Digits, South Asian Grouping, Dates, Words)
│   │   │   └── Migrations/                    (create_translator_engine_locales_table)
│   │   │
│   │   ├── AutomationAI/                      # Module 4 (Type 9)
│   │   │   ├── Drivers/                       (AiDriver)
│   │   │   └── Providers/                     (GeminiProvider, OpenAiProvider, DeepLProvider)
│   │   │
│   │   └── HeadlessApi/                       # Module 5 (Type 10)
│   │       ├── Controllers/                   (TranslatorEngineApiController)
│   │       ├── Middleware/                    (TranslatorEngineLocaleMiddleware)
│   │       └── Routes/                        (api.php)
│   │
│   └── Console/
│       ├── InstallCommand.php                 # php artisan translator-engine:install
│       ├── MakeExternalCommand.php            # php artisan translator-engine:make:external {table}
│       ├── MakeHybridCommand.php              # php artisan translator-engine:make:hybrid {domain}
│       ├── MakeInlineCommand.php              # php artisan translator-engine:make:inline {table} {column}
│       └── AiSyncCommand.php                  # php artisan translator-engine:ai-sync
```
